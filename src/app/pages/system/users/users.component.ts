  ...
  editUser(user: UserData): void {
    ...
    dialog_ref.afterClosed().subscribe((dialog_response: DialogResponse) => {
      if (dialog_response && dialog_response.operation === "user.edit") {
        ...
        this._http_service.put_users_user_id
          .sendRequest(user_data._id, user_data)
          .subscribe(
            (res: ApiResponse) => {
              ...
              if (
                this.mode.editing_user_ids.includes(dialog_response.data._id)
              ) {
                this.mode.editing_user_ids.splice(
                  this.mode.editing_user_ids.indexOf(res.data._id),
                  1
                );
              }
            },
            (error: Error) => {
              ...
              if (
                ...
                this.mode.editing_user_ids.splice(
                  this.mode.editing_user_ids.indexOf(dialog_response.data._id),
                  1
                );
              }
            }
              ...
              (res: ApiResponse) => {
                for (const user_data of this.users_data) {
                  if (user_data._id === user._id) {
                    this.users_data.splice(
                      this.users_data.indexOf(user_data),
                      1
                    );
                    break;
                  }
